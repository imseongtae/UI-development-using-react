...
import { formatRelativeDate } from 'data/helpers';
...
const List:React.FunctionComponent<Props> = ({ data = [], hasIndex, hasDate, onClick, onRemove }) => {
    ...
    <ul className="list">
      {data.map(({id , keyword, date}, index) => (
        <li key={index} onClick={() => onClick(keyword)}>
          ...
          {!!onRemove && (
            <button className="btn-remove" onClick={event => handleClickRemove(event, keyword)} />
          )}
        </li>
      ))}
    </ul>
